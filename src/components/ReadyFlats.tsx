...
export default function ReadyFlats() {
  const { t } = useTranslation();
  const properties = (t('propertyData') as any[]).filter((p: any) => p.status === 'Ready' || p.status === 'প্রস্তুত');

  return (
    <section id="properties" className="py-16 md:py-24 lg:py-32 bg-brand-sand relative overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 w-96 h-96 bg-brand-mint/20 blur-[100px] rounded-full translate-y-1/2 -translate-x-1/2" />
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          ...
          >
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-serif font-extrabold text-brand-charcoal leading-[1.15] sm:leading-tight">
              {t('propertiesTitle')} <br className="hidden sm:block" />
              <span className="text-brand-green font-medium">{t('propertiesTitleSpan')}</span>
            </h2>
          </motion.div>
          <motion.p 
            initial={{ opacity: 0, y: 30 }}
